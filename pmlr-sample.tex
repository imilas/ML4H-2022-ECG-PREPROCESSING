...
{\caption{Functions used by Top 5 teams in Physionet2020~\citep{hong2022practical}}}%
    ...
    1    & Wide and Deep Transformer~\citep{natarajan2020wide}                   &                           & \checkmark & \checkmark & 15 sec.\\ \hline
    2    & Adaptive ResNet~\citep{zhao2020adaptive}                  & \checkmark (257 Hz) & \checkmark & & 16 sec.\\ \hline
    3    & SE-ResNet~\citep{zhu2020classification}        &                           &                           &                           & 30 sec.\\ \hline
    4    & Scatter Transform and DNNs ~\citep{oppelt2020combining}             &                           & \checkmark &                           & 20.5 sec. \\ \hline
...
\subsection{Resizing} 
Signals are time-series, and time-series vary in length. Resizing refers to assigning a fixed length (or window) to all signals. Here, we use a fixed window length for all experiments, and samples with zeros on the right, if the full size $x$ is lower than our desired fixed length. If $x$ is longer than the fixed length, then samples 0-$x$ are used. 
